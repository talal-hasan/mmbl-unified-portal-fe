...
import { authGuard, roleGuard as inlineRoleGuard } from './core/guards/auth.guard';
import { roleGuard }       from './core/guards/role.guard';
import { permissionGuard } from './core/guards/permission.guard';

/**
 * Three guard options — mix and match as needed:
 *
 * 1. authGuard          → user must be logged in (any role)
 * 2. roleGuard          → data-driven: data: { roles: ['admin'] }
 * 3. permissionGuard    → data-driven: data: { permission: 'manage-users' }
 */
export const routes: Routes = [
  ...
  // Role-restricted — admin only (data-driven)
  {
    path: 'admin',
    loadComponent: () =>
      import('./features/dashboard/dashboard.component').then(m => m.DashboardComponent),
  ...
  // Permission-restricted — needs 'manage-users' permission
  {
    path: 'users',
    loadComponent: () =>
  ...
  // Permission-restricted — needs 'view-reports' permission
  {
    path: 'reports',
    loadComponent: () =>
...
//         path: 'team-dashboard',
//         loadComponent: () =>
//           import('./features/team-management/team-management').then((m) => m.TeamManagement),
//         canActivate: [permissionGuard],
//         data: { permission: 'team-dashboard' },
//       },
//       {
//         path: 'users',
//         loadComponent: () =>
